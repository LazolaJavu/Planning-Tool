...
model_data =  pd.read_excel(r"C:/Users/LazolaJavu/OneDrive - SmartStart/Documents/SmartStart/Ops Department/Mapaseka's Request'/Planning-Tool/Models/Planning_Model v.01.xlsx", sheet_name="Model")

due_pqa_df = pd.read_excel(r"C:/Users/LazolaJavu/OneDrive - SmartStart/Documents/SmartStart/Ops Department/Mapaseka's Request'/Planning-Tool/Models/Planning_Model v.01.xlsx", sheet_name="Due_PQA")
...
def franchisor():
	...
	AR = st.sidebar.slider("Due for Annual Reaccreditation (hrs)", min_value=2, max_value=10, value=2, step=1)
	...
	Activity_1 = (col2.number_input('Enter your Club Meetings Planned', min_value=0, max_value=100, step=1)*Club_Meeting)/8
	Activity_2 = (col2.number_input('Enter your AR Planned', min_value=0, max_value=100, step=1)*AR)/8
	Activity_3 = (col2.number_input('Enter your PQA Planned', min_value=0, max_value=100, step=1)*PQA)/8
	Activity_4 = (col2.number_input('Enter your Smartspace Planned', min_value=0, max_value=100, step=1)*Smartspace)/8
	Activity_5 = (col2.number_input('Enter your PrePQA Planned', min_value=0, max_value=100, step=1)*PrePQA)/8
	Activity_6 = (col2.number_input('Enter your Coaching Circle Planned', min_value=0, max_value=100, step=1)*Coaching_Circle)/8
	...
	Activity_11 = (col3.number_input('Enter your Matching Day Planned', min_value=0, max_value=100, step=1)*Matching_Day)/8
	Activity_12 = (col3.number_input('Enter your PreStartup Planning Planned', min_value=0, max_value=100, step=1)*PreStartup_Planning)/8
	Activity_13 = (col3.number_input('Enter your Startup Training Planned', min_value=0, max_value=100, step=1)*Startup_Training)/8
	...
	with col6:
		...
		st.metric(label="Total Days Left", value=round(sum_by_category.loc['Total Days', select_box1] -added_activities, 1))
	...
	if col6.button('Download Due PQA - Franchisees'):

		df = due_pqa_df[due_pqa_df['Franchisor'] == select_box1]

		df.to_excel(f'Due for PQA {select_box1}.xlsx', index=False)
	...
	with col5:
		...
		# Create a DataFrame with the entered data
		data = {'Club Meetings': sum_by_category.loc['Clubs Eligible for Meeting', select_box1] - [(Activity_1*8)/Club_Meeting], 'AR': sum_by_category.loc['Due for Annual Reaccreditation', select_box1] - [(Activity_2*8)/AR], 'PQA': sum_by_category.loc['Due For PQA', select_box1] - [(Activity_3*8)/PQA],
		'Smartspace': sum_by_category.loc['Can be SmartSpaced', select_box1] - [(Activity_4*8)/Smartspace], 'PrePQA': [(Activity_5*8)/PrePQA], 'Coaching Circle': [(Activity_6*8)/Coaching_Circle],
		...
		'PreStartup Planning': [(Activity_12*8)/PreStartup_Planning], 'Startup Training': [(Activity_13*8)/Startup_Training], 'CP Planning': sum_by_category.loc['Due Child Progress Sessions', select_box1] - [(Activity_14*8)/CP_Planning],
		'BS Planning': sum_by_category.loc['Due Business Skill Sessions', select_box1] - [(Activity_15*8)/BS_Planning], 'Consolidation': [(Activity_16*8)/Consolidation], 'Briefings Coach Meetings': [(Activity_17*8)/Briefings_Coach_Meetings],
		'Support Time': [(Activity_18*8)/Coach_Support_Time]}
		df = pd.DataFrame(data)
		df = df.transpose().round(0).astype(int)

		# Rename the second column by its index (0-based index)
		new_column_name = 'Numbers'
		column_index_to_rename = 0  # Index of the column to rename (0 for the first column, 1 for the second, etc.)
...
# Coach_Support_Time = st.sidebar.slider("Coach Support Time", min_value=8, max_value=15, value=8, step=1)
	...
	cols2.metric(label="Can be SmartSpaced", value=sum_by_category.loc['Can be SmartSpaced', select_box1] - ((Activity_4*8)/Smartspace))
	cols3.metric(label="Due For PQA", value=round(sum_by_category.loc['Due For PQA', select_box1], 0))
	cols4.metric(label="Clubs Eligible for Meeting", value=round(sum_by_category.loc['Clubs Eligible for Meeting', select_box1], 0))
	...
	cols2.metric(label="Due for Annual Reaccreditation", value=round(sum_by_category.loc['Due for Annual Reaccreditation', select_box1], 0))
	cols3.metric(label="Support Days", value=round(sum_by_category.loc['Support Days', select_box1], 0))
	cols4.metric(label="Due Child Progress Sessions", value=round(sum_by_category.loc['Due Child Progress Sessions', select_box1], 0))
	cols5.metric(label="Due Business Skill Sessions", value=round(sum_by_category.loc['Due Business Skill Sessions', select_box1], 0))
	# Third Row
	cols1.metric(label="Trainings to Complete", value=round(sum_by_category.loc['Trainings to Complete', select_box1], 0))
...
def coach_specific():
	...
	with col6:
		...
		st.metric(label="Total Days Left", value=round(final_data['Total Days'] -added_activities, 1), delta=-added_activities, delta_color="inverse")
	...
	if col4.button('Save Plan'):
		...
		data = {'Club Meetings': [(Activity_1*8)/Club_Meeting], 'AR': [(Activity_2*8)/AR], 'PQA':  [(Activity_3*8)/PQA],
		'Smartspace':  [(Activity_4*8)/Smartspace], 'PrePQA': [(Activity_5*8)/PrePQA], 'Coaching Circle': [(Activity_6*8)/Coaching_Circle],
		'Club leader meetings': [(Activity_7*8)/Club_leader_meetings], 'Porridge': [(Activity_8*8)/Porridge],'Coach Admin Time': [(Activity_9*8)/Coach_Admin_Time],
		'Community Engagement for Matching': [(Activity_10*8)/Community_Engagement_for_Matching], 'Matching Day': [(Activity_11*8)/Matching_Day],
		'PreStartup Planning': [(Activity_12*8)/PreStartup_Planning], 'Startup Training': [(Activity_13*8)/Startup_Training], 'CP Planning':  [(Activity_14*8)/CP_Planning],
		'BS Planning': [(Activity_15*8)/BS_Planning], 'Consolidation': [(Activity_16*8)/Consolidation], 'Briefings Coach Meetings': [(Activity_17*8)/Briefings_Coach_Meetings],
		'Support Time': [(Activity_18*8)/Coach_Support_Time]}
	...
	if col6.button('Download Due PQA - Franchisees'):
		...
		df = due_pqa_df[due_pqa_df['Coach'] == select_box2]

		df.to_excel(f'Due for PQA {select_box2}.xlsx', index=False)

	with col5:


		# Create a DataFrame with the entered data
		data = {'Club Meetings': round(final_data['Clubs Eligible for Meeting'] - [(Activity_1*8)/Club_Meeting],1), 'AR': final_data['Due for Annual Reaccreditation'] - [(Activity_2*8)/AR], 'PQA': final_data['Due For PQA'] - [(Activity_3*8)/PQA],
		'Smartspace': final_data['Can be SmartSpaced'] - [(Activity_4*8)/Smartspace], 'PrePQA': [(Activity_5*8)/PrePQA], 'Coaching Circle': [(Activity_6*8)/Coaching_Circle],
		'Club leader meetings': [(Activity_7*8)/Club_leader_meetings], 'Porridge': [(Activity_8*8)/Porridge],'Coach Admin Time': [(Activity_9*8)/Coach_Admin_Time],
		'Community Engagement for Matching': [(Activity_10*8)/Community_Engagement_for_Matching], 'Matching Day': [(Activity_11*8)/Matching_Day],
		'PreStartup Planning': [(Activity_12*8)/PreStartup_Planning], 'Startup Training': [(Activity_13*8)/Startup_Training], 'CP Planning': final_data['Due Child Progress Sessions'] - [(Activity_14*8)/CP_Planning],
		'BS Planning': final_data['Due Business Skill Sessions'] - [(Activity_15*8)/BS_Planning], 'Consolidation': [(Activity_16*8)/Consolidation], 'Briefings Coach Meetings': [(Activity_17*8)/Briefings_Coach_Meetings],
		'Support Time': [(Activity_18*8)/Coach_Support_Time]}
		df = pd.DataFrame(data)
		df = df.transpose().round(0).astype(int)
		# Rename the second column by its index (0-based index)
	...
	# st.write(sum_by_category[select_box1])
	cols1.metric(label="Active Franchisees", value=round(sum_by_category.loc['Active Franchisees', select_box1], 0))
	cols2.metric(label="Can be SmartSpaced", value=round(sum_by_category.loc['Can be SmartSpaced', select_box1]  - ((Activity_4*8)/Smartspace), 0))
	cols3.metric(label="Due For PQA", value=round(sum_by_category.loc['Due For PQA', select_box1], 0))
	cols4.metric(label="Clubs Eligible for Meeting", value=round(sum_by_category.loc['Clubs Eligible for Meeting', select_box1], 0))
